import pytest
from src.core.checker import calculate_ratio, determine_alert_level, _parse_ohlcv
...
def test_parse_ohlcv_sums_volumes():
    data = {"t": [1, 2, 3], "v": [100, 200, 300], "c": [50.0, 51.0, 52.0]}
    vol, close = _parse_ohlcv(data)
    assert vol == 600
    assert close == 52.0


def test_parse_ohlcv_none_data():
    vol, close = _parse_ohlcv(None)
    assert vol == 0
    assert close is None


def test_parse_ohlcv_empty_candles():
    vol, close = _parse_ohlcv({"t": [], "v": [], "c": []})
    assert vol == 0
    assert close is None
...
def test_ratio_at_midday():
    # elapsed=127 (≈ half day), vma9=1_000_000, vol=500_000 → ratio ≈ 1.0
...
@pytest.mark.parametrize("ratio,session,level", [
    ...
    (2.5,  "ATO",       None),               # no warning in ATO
    # MORNING / AFTERNOON / ATC
...
def test_warning_to_critical_escalation():
    """WARNING and CRITICAL use separate Redis keys so escalation always fires."""
